import { decimalToBinary, binaryToDecimal } from "./binaryUtils";
...
function generateDecimalChoices(binaryString: string): DecimalChoice[] {
    const correctDecimal = binaryToDecimal(binaryString);
    const choices: DecimalChoice[] = [{ value: correctDecimal, isCorrect: true }];
    const usedValues = new Set<number>([correctDecimal]);
    const wrongChoices: DecimalChoice[] = [];
    ...
    for (let i = 0; i < binaryString.length; i++) {
        ...
        if (!usedValues.has(flippedDecimal)) {
            wrongChoices.push({ value: flippedDecimal, isCorrect: false });
